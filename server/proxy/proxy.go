...
func (pxy *BaseProxy) GetWorkConnFromPool(src, dst net.Addr) (workConn net.Conn, err error) {
	...
	for i := 0; i < pxy.poolCount+1; i++ {
		...
		xl.Tracef("get a new work connection: [%s]", workConn.RemoteAddr().String())
			...
			for {
				...
				if err != nil {
					if err, ok := err.(interface{ Temporary() bool }); ok && err.Temporary() {
						...
						xl.Tracef("met temporary error: %s, sleep for %s ...", err, tempDelay)
...
func (pxy *BaseProxy) handleUserTCPConnection(userConn net.Conn) {
	...
	xl.Tracef("join connections, workConn(l[%s] r[%s]) userConn(l[%s] r[%s])", workConn.LocalAddr().String(),
