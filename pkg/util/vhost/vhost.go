...
func (v *Muxer) handle(c net.Conn) {
	...
	if err != nil {
		logx.Verbosef("get hostname from http/https request error: %v", err)
		_ = c.Close()
		return
	}
	...
	if !ok {
		logx.Verbosef("http request for host [%s] path [%s] httpUser [%s] not found", name, path, httpUser)
		v.failHook(sConn)
		return
	}
	...
	if l.mux.checkAuth != nil && l.username != "" {
		...
		if !ok || err != nil {
			xl.Tracef("auth failed for user: %s", l.username)
			_ = c.Close()
			return
		}
	}
	...
	xl.Tracef("new request host [%s] path [%s] httpUser [%s]", name, path, httpUser)
...
func (l *Listener) Accept() (net.Conn, error) {
	...
	if l.mux.rewriteHost != nil {
		...
		xl.Tracef("rewrite host to [%s] success", l.rewriteHost)
		conn = sConn
	}
