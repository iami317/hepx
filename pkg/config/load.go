...
func LoadConfig(allCfg *v1.ClientConfig) (*v1.ClientCommonConfig, []v1.ProxyConfigurer, []v1.VisitorConfigurer) {
	var (
		commonCfg   *v1.ClientCommonConfig
		proxyCfgs   = make([]v1.ProxyConfigurer, 0)
		visitorCfgs = make([]v1.VisitorConfigurer, 0)
	)
	commonCfg = &allCfg.ClientCommonConfig
	...
	for _, c := range proxyCfgs {
		c.Complete(commonCfg.User)
	}
	for _, c := range visitorCfgs {
		c.Complete(commonCfg)
	}
	return commonCfg, proxyCfgs, visitorCfgs
}

func LoadClientConfig(path string, strict bool) (*v1.ClientCommonConfig, []v1.ProxyConfigurer, []v1.VisitorConfigurer, error) {
	var (
		commonCfg   *v1.ClientCommonConfig
		proxyCfgs   = make([]v1.ProxyConfigurer, 0)
		visitorCfgs = make([]v1.VisitorConfigurer, 0)
	...
	// Filter by start
	if len(commonCfg.Start) > 0 {
		startSet := sets.New(commonCfg.Start...)
	...
	return commonCfg, proxyCfgs, visitorCfgs, nil
}
