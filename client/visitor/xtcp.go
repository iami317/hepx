...
func (sv *XTCPVisitor) keepTunnelOpenWorker() {
	...
	for {
		...
		case <-sv.ctx.Done():
			return
		case <-ticker.C:
			xl.Tracef("keepTunnelOpenWorker try to check tunnel...")
			...
			xl.Tracef("keepTunnelOpenWorker check success")
			if conn != nil {
				conn.Close()
			}
		}
...
func (sv *XTCPVisitor) handleConn(userConn net.Conn) {
	...
	xl.Tracef("get a new xtcp user connection")
	...
	if err != nil {
		...
		xl.Tracef("try to transfer connection to visitor: %s", sv.cfg.FallbackTo)
